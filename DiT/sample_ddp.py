...
def main(args):
    ...
    # assert args.cfg_scale >= 0.0 # Zhengqi: I have revised the code, see forward_with_cfg in models.py
    # using_cfg = args.cfg_scale > 0.0
    ...
    folder_name = (f"{model_string_name}-{ckpt_string_name}-size-{args.image_size}-vae-{args.vae}-" \
                  f"scale-{args.cfg_scale}-seed-{args.global_seed}-sample-{int(args.num_fid_samples/1e3)}k-"
                  f"type-{args.cfg_type}-param-{'_'.join([str(p) for p in args.cfg_param])}")
    ...
    for _ in pbar:
        ...
        if using_cfg:
            ...
            model_kwargs = dict(y=y, cfg_param=construct_cfg(args.cfg_type, args.cfg_scale, *args.cfg_param))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg-type", type = str, default = 'original3')
    parser.add_argument("--cfg-param", nargs='+', default = [])
